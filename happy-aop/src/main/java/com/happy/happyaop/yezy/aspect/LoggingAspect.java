...
//    @Before("within(com.example.aop.controller.AopController)")
    // @annotation: 어노테이션 지정
    @After("@annotation(com.happy.happyaop.yezy.aspect.LogExecutionTime)")
    ...
    @Around("@annotation(com.happy.happyaop.yezy.aspect.LogExecutionTime)")
    public Object logExectionTime(
            // Advice 내에서 대상 JoinPoint가 실행되도록 요구할 수 있다.
            ProceedingJoinPoint joinPoint
    ) throws Throwable {
        log.info("start log execution time");
        long startTime = System.currentTimeMillis();

        // joinPoint.proceed(): JoinPoint에 해당하는 메소드를 진행해주세요.
        Object proceed = joinPoint.proceed();

        long endTime = System.currentTimeMillis();

        // 실제로 추가하고 싶은 기능
        log.info("{} excuted in: {}ms", joinPoint.getSignature(), endTime - startTime);
        return proceed;
    }
}